...
void main() {
  group('ModbusClient', () {
    ...
    test(
      'falls back from multiple-register write to single-register writes',
      () async {
        final server = await ModbusTcpTestServer.start(
    ...
    test('connects to RTU/IP and reads registers and bits', () async {
      ...
      addTearDown(server.close);
      final client = ModbusClient(_rtuDevice(server));
      addTearDown(client.disconnect);
    ...
    test(
      'writes RTU/IP single and multiple holding registers and coils',
      () async {
        final server = await ModbusRtuIpTestServer.start();
        addTearDown(server.close);
        final client = ModbusClient(_rtuDevice(server));
    ...
    test('maps RTU/IP Modbus exception responses', () async {
      final server = await ModbusRtuIpTestServer.start(
        exceptionByFunction: {0x03: 0x02},
      );
      addTearDown(server.close);
    ...
    test('falls back from RTU/IP multiple-register write', () async {
      final server = await ModbusRtuIpTestServer.start(
        illegalMultipleRegisterWrite: true,
      );
      addTearDown(server.close);
    ...
    test('logs RTU/IP TX and full RX frames with CRC', () async {
      final log = TrafficLog.instance;
      await log.resetForTesting();
      addTearDown(log.resetForTesting);
      final server = await ModbusRtuIpTestServer.start(
        holdingRegisters: {0: 0x1234},
      );
      addTearDown(server.close);
      final device = _rtuDevice(server);
      ...
      final entries = log.entriesFor(device.id);
      expect(entries, hasLength(2));
      expect(entries[0].direction, LogDirection.tx);
      expect(entries[0].frameKind, LogFrameKind.modbusRtu);
      expect(entries[1].direction, LogDirection.rx);
      expect(entries[1].frameKind, LogFrameKind.modbusRtu);
      expect(entries[1].data, startsWith('01 03 02 12 34'));
    });
  });
}
...
DeviceInfo _rtuDevice(ModbusRtuIpTestServer server) => DeviceInfo(
  name: 'RTU PLC',
  host: server.host,
  port: server.port,
