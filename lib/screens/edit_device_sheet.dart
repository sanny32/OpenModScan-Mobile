...
class EditDeviceSheet extends StatefulWidget {
  ...
  const EditDeviceSheet({super.key, required this.device});
...
class _EditDeviceSheetState extends State<EditDeviceSheet> {
  ...
  void _save() {
    final port = int.tryParse(_portCtrl.text) ?? widget.device.port;
    final unitId = int.tryParse(_unitCtrl.text) ?? widget.device.unitId;
    final updated = widget.device.copyWith(
      name: _nameCtrl.text.trim().isEmpty ? widget.device.name : _nameCtrl.text.trim(),
      host: _hostCtrl.text.trim().isEmpty ? widget.device.host : _hostCtrl.text.trim(),
      port: port,
      protocol: _connType == 0 ? ProtocolType.modbusTcp : ProtocolType.modbusRtuIp,
      unitId: unitId,
